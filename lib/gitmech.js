...
var mediaSubdir // MOD add variable for media static content
var gitCommandsQuote // MOD add variable for quoting absolute paths
...
// MOD fixed gitExec function for concatenating paths with blank spaces
function gitExecFix (commands, callback) {
  if (gitCommandsQuote){
    commands = gitMech.gitBin + ' ' + gitCommandsQuote.concat(commands).join(' ')
  } else {
    commands = gitMech.gitBin + ' ' + gitCommands.concat(commands).join(' ')
  }
  // There is a limit at 200KB (increase it with maxBuffer option)
...
var gitMech = {
  ...
  setup: function (gitBin, repoDir, repoDocSubdir, repoMediaSubdir, refspec, callback) { // MOD add media arg
    ...
    childProcess.exec(this.gitBin + ' --version', function (err, stdout, stderr) {
      ...
      // MOD add validation of media subdirectory
      mediaSubdir = repoMediaSubdir.trim().replace(/^\/|\/$/g, '')
      if (mediaSubdir !== '') {
        mediaSubdir = mediaSubdir + '/'
      }
      // MOD report usage of media subdirectory
      if (mediaSubdir){
        try {
          fs.statSync(repoDir + '/' + mediaSubdir)
          console.log('Using media subdirectory at ' + repoDir + '/' + mediaSubdir)
        } catch (e) {}
      }
      ...
      try {
        ...
        fs.statSync(gitDir)
        workTree = repoDir
        // MOD construct alternate commands for paths with blank space
        var quote_worktree = '"' + workTree + '"'
        var quote_gitdir = '"' + gitDir + '"'
        if (/\s/.test(gitDir)){
            gitCommandsQuote = ['--git-dir=' + quote_gitdir, '--work-tree=' + quote_worktree]
        }
  ...
  // MOD add media path to file
  mediaPath: function (path='') {
    if (mediaSubdir){
        return workTree + '/' + mediaSubdir + path
    } else {
        return null
    }
  },
  ...
  ls: function (pattern, callback) {
    // MOD fix function to use quoted paths
    gitExecFix([ 'ls-tree', '--name-only', '-r', 'HEAD', '--', docSubdir + pattern ], function (err, data) {
      if (err) {
        data = ''
      }
