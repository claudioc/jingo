...
//var methodNamesToPromisify = "readFile show remoteExists pull push log hashes add rm commit grep diff lastMessage ls".split(" ");
...
var gitMech = {
  ...
  setup: function(gitBin, repoDir, repoDocSubdir, refspec, callback) {
    ...
    childProcess.exec(gitBin + " --version", function (err, stdout, stderr) {

      if (err !== null || stderr !== "") {
        callback("Cannot run git (tried with " + gitBin + ")");
        return;
      }
      ...
      if (version[0] != "git" || version.length < 3) {
        callback("The provided git binary (" + gitBin + ") doesn't seem git to me.");
        return;
      }
  ...
  pull: function(callback) {
    ...
    this.remoteExists(this.remote, function(err, exists) {
      ...
      gitExec(["pull", this.remote, this.branch], function(err) {
        this.pulling = false;
  ...
  push: function(callback) {
    ...
    this.remoteExists(this.remote, function(err, exists) {
      ...
      gitExec(["push", this.remote, this.branch], function(err) {
        this.pushing = false;
        if (err && err.toString().match(/^Error:/)) {
          var lines = err.toString().split("\n");
  ...
  mv: function(path, newPath, message, author, callback) {
    gitExec(["mv", "-f", docSubdir + path,  docSubdir + newPath], function(err) {
      if (err) {
        console.log(err);
        callback(err);
      } else {
