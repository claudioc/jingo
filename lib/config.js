...
module.exports = (function () {
  ...
  return {
    ...
    defaults: {
      ...
      // MOD Map of terms and the article each redirects to
      redirects: {},
      
      // MOD Configuration of elements in pug layouts
      layout: {
        sidebarColumn: 3,
        mainColumn: 9,
        footerColumn: 9,
      ...
      // MOD RegExp to use for redaction of content
      redaction: {
        hiddenPage: '/\\s?Hidden/i',
        privateSection: '/\\s?Private/i',
        earliestDate: '/\\s\\d{4}\\.\\d{2}\\.\\d{2}/, /\\s\\d{4}\\-\\d{2}\\-\\d{2}/',
        endRedaction: '/\\sEnd\\s/i',
        sequentialRollout: [{
            expression: '',
            currentValue: 0
        }]
    ...
    validate: function () {
      ...
      // MOD add validation of layout fields
      config.layout = _.extend({}, this.defaults.layout, config.layout)
      config.layout.sidebarColumn = parseInt(config.layout.sidebarColumn, 10)
      config.layout.sidebarColumn = config.layout.sidebarColumn < 11 ? config.layout.sidebarColumn : 3
      config.layout.sidebarColumn = config.layout.sidebarColumn >= 0 ? config.layout.sidebarColumn : 3
      config.layout.mainColumn = parseInt(config.layout.mainColumn, 10)
      config.layout.mainColumn = config.layout.mainColumn < 13 ? config.layout.mainColumn : 9
      config.layout.mainColumn = config.layout.mainColumn > 1 ? config.layout.mainColumn : 9
      if (config.layout.sidebarColumn + config.layout.mainColumn > 12){
        error = 'Total width of sidebarColumn + mainColumn must not exceed 12.'
        return false
      }
      config.layout.footerColumn = parseInt(config.layout.footerColumn, 10)
      config.layout.footerColumn = config.layout.footerColumn < 13 ? config.layout.footerColumn : 9
      config.layout.footerColumn = config.layout.footerColumn > 0 ? config.layout.footerColumn : 9
      
      // MOD extend redaction and redirection
