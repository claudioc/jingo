  ...
  return {
    ...
    load: function (filename) {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        // MOD Allow unlimited keys in redirects section as long as they have string values
        if (keys[i] == 'redirects'){
            for (var k in config['redirects']) {
                if (typeof config['redirects'][k] !== 'string'){
                    error = 'Value for configuration field ' + k + ' in section redirects must be a string.'
                    return false 
                }
            }
        } else {
    ...
    defaults: {
      ...
      features: {
        ...
        caseSensitive: false // MOD enable case sensitivity in search and redirection
      },
      ...
      // MOD map of terms and the page each redirects to
      redirects: {
        index: 'Home.md'
      },
      ...
      redaction: {
        ...
        privateSection: '<!(--\\s?Private[\\s\\S]*?--)>',
        earliestDate: '<!(--\\s\\d{4}\\.\\d{2}\\.\\d{2}[\\s\\S]*?--)>',
        sequentialSections: [{
          expression: '<!(--\\schapter-\\d+[\\s\\S]*?--)>',
          latestValue: 0
        }],
        endRedaction: '<!(--\\sEnd\\s--)>'
      }
    ...
    validate: function () {
      ...
      // MOD extend redirection
      config.redirects = _.extend({}, this.defaults.redirects, config.redirects)
      
      // MOD validate redaction fields
      config.redaction = _.extend({}, this.defaults.redaction, config.redaction)
      var regex_fields = [ 'endRedaction', 'hiddenPage', 'privateSection', 'earliestDate' ]
      for (var i = 0; i < regex_fields.length; i++){
        var regex_value = config.redaction[regex_fields[i]]
        if (regex_value){
          try {
            new RegExp(regex_value)
          } catch(e) {
            error = 'Value for configuration field ' + regex_fields[i] + ' in section redaction must be a valid RegExp string.'
            return false
          }
          if (i > 1 && !config.redaction.endRedaction){
            error = 'Use of configuration field ' + regex_fields[i] + ' in section redaction requires specifying an endRedaction value.'
            return false
          }
        }
      }
      if (config.redaction.sequentialSections){
        for (var i = 0; i < config.redaction.sequentialSections.length; i++){
          var latest_section = config.redaction.sequentialSections[i]
          var error_prefix = 'Value for configuration field sequentialSections item [' + i.toString() + '] key expression in section redaction'
          if (latest_section.expression){
            try {
              new RegExp(latest_section.expression)
            } catch(e) {
              error = error_prefix + ' must be a valid RegExp string.'
              return false
            }
            if (!(/\\d\+/.test(latest_section.expression))){
                error = error_prefix + ' must contain the term \\d+.'
                return false
            }
          }
        }
        if (!config.redaction.endRedaction){
          error = 'Use of configuration field ' + regex_fields[i] + ' in section redaction requires specifying an endRedaction value.'
          return false
        }
      }
