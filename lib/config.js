...
module.exports = (function () {
  ...
  return {
    ...
    defaults: {
      ...
      redaction: {
        enabled: true,
        hiddenPage: '/^<!(--\\s?Hidden[\\s\\S]*?--)>/i',
        privateSection: '/<!(--\\s?Private[\\s\\S]*?--)>/i',
    ...
    validate: function () {
      ...
      for (var i = 0; i < config.redaction.rolloutSections.length; i++){
        const rollout_section = config.redaction.rolloutSections[i]
        if (rollout_section.expression){
            try {
              new RegExp(rollout_section.expression)
            } catch(e) {
              error = 'Value for configuration field rolloutSections item [' + i.toString() + '] expression in section redaction must be a valid RegExp string.'
              return false
            }
        }
      }
