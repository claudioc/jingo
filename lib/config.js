...
module.exports = (function () {
  ...
  return {
    ...
    validate: function () {
      ...
      var regex_fields = [ 'hiddenPage', 'privateComment', 'earliestDate' ]
      ...
      if (config.redaction.sequentialSections){
        for (var i = 0; i < config.redaction.sequentialSections.length; i++){
          ...
          if (latest_section.expression){
            ...
            if (typeof(latest_section.latestValue) != 'number'){
                error = error_prefix + ' requires a latestValue be included.'
                return false
            }
          }
        }
