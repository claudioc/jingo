...
module.exports.initialize = function (config) {
  ...
  // MOD use favicon.ico in media folder
  if (Git.mediaPath('favicon.ico') && fs.existsSync(Git.mediaPath('favicon.ico'))){
    app.use(favicon(Git.mediaPath('favicon.ico')))
  } else {
  ...
  if (Git.mediaPath()){
    app.use(express.static(Git.mediaPath())) // MOD files in media path to static files
  }
