...
module.exports.initialize = function(config, auth) {
  ...
  var pc = app.locals.pagesConfig = config.get("pages", { namingConventions: "lowercase,dash", index: "home"});

  if (!pc.namingConventions) {
    pc.namingConventions = "dash";
  }
  if (pc.namingConventions.indexOf("dash") == -1 && pc.namingConventions.indexOf("underscore") == -1) {
    pc.namingConventions += ",dash";
  }
  if (~pc.namingConventions.indexOf("dash") && ~pc.namingConventions.indexOf("underscore")) {
    pc.namingConventions = pc.namingConventions.replace(/underscore/g, '');
  }
  if (!pc.index) {
    pc.index = "home";
  }
