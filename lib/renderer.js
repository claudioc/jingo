...
function renderPageList(text) {
  ...
  if (matches) {
    ...
    var pages = GLOBAL.pageCache.filter(function(p) { return (p.name !== configuration.getConfig().customizations.sidebar.replace(".md","") && p.name !== configuration.getConfig().customizations.footer.replace(".md","") && p.name !== configuration.getConfig().pages.index) })
