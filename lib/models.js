...
Pages.prototype.fetchAll = function() {
    ...
    setPageCache: function() {
      ...
      gitmech.ls("*.md", function(err, list) {
        var listWithData = list.map(function (page) {
          ...
          return {
            file: page,
            name: path.basename(page).replace(/\.md$/,""),
            stats: stats
          };
        });

        // set the cache to our reference variable
        GLOBAL.pageCache = listWithData;
      });
    }
